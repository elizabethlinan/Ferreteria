...
public class ClienteMySqlRepositorio implements ClienteRepositorio{
	...
	public int inserta(Cliente obj) {
		 return jdbcTemplate.update("insert into cliente values(null,?,?,?,?,?)", new Object[]
				 { obj.getNombre(), obj.getApellido(), obj.getDni(), obj.getTelefono(),obj.getDireccion()});
	...
	public List<Cliente> lista(String filtro) {
		List<Cliente> lista = jdbcTemplate.query("select * from cliente where nombre like ? ",
		new Object[] { filtro+"%" }, new RowMapper<Cliente>() {
            @Override
