...
public class ProductoMySqlRepositorio implements ProductoRepositorio{
	...
	public int inserta(Producto obj) {
		 return jdbcTemplate.update("insert into producto values(null,?,?,?,?)",
				 new Object[] { obj.getNombre(), obj.getMarca(), obj.getStock(), obj.getEstado()});
	}
	...
	public List<Producto> lista(String filtro) {
		List<Producto> lista = jdbcTemplate.query("select * from producto where nombre like ? ",
				new Object[] { filtro+"%" }, new RowMapper<Producto>() {
            @Override
