...
public class UsuarioMySqlRepositorio implements UsuarioRepositorio {
	...
	public int inserta(Usuario obj) {
		...
		return jdbcTemplate.update("insert into usuario values(null,?,?,?,?,?)",
	new Object[] { obj.getNombre(), obj.getApellido(), obj.getUsuario(), obj.getContraseña(), obj.getEmail()});
	}
	...
	public List<Usuario> lista(String filtro) {
		...
		List<Usuario> lista = jdbcTemplate.query("select * from usuario where nombre like ? "
				,new Object[] { filtro+"%" }, new RowMapper<Usuario>() {
            @Override
            public Usuario mapRow(ResultSet rs, int arg1) throws SQLException {
            	...
            	obj.setContraseña(rs.getString("Contrase�a"));
